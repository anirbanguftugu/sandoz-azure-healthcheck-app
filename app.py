...
# -----------------------------
# Azure OpenAI / OpenAI API key handling
# -----------------------------
# NOTE: original code used OPENAI_API_KEY from env; that block is commented below and replaced
# with Azure-specific environment variables. We only change env/key handling — no other logic is touched.
...
# Read Azure OpenAI settings from .env (you will create this .env)
AZURE_OPENAI_API_KEY = os.environ.get("AZURE_OPENAI_API_KEY") or os.environ.get("AZURE_API_KEY")
AZURE_OPENAI_ENDPOINT = os.environ.get("AZURE_OPENAI_ENDPOINT") or os.environ.get("AZURE_OPENAI_BASE")
AZURE_OPENAI_API_VERSION = os.environ.get("AZURE_OPENAI_API_VERSION") or os.environ.get("OPENAI_API_VERSION")
# Optional: deployment name (if your Azure deployment uses a different model name)
AZURE_OPENAI_DEPLOYMENT = os.environ.get("AZURE_OPENAI_DEPLOYMENT") or os.environ.get("AZURE_DEPLOYMENT_NAME")

if not AZURE_OPENAI_API_KEY or not AZURE_OPENAI_ENDPOINT:
    st.warning(
        "Azure OpenAI settings are not fully set. Please add AZURE_OPENAI_API_KEY and AZURE_OPENAI_ENDPOINT in your .env. 🔑"
    )
else:
    # Set environment variables that OpenAI SDK / LangChain will consume for Azure.
    # We set OPENAI_API_KEY so existing ChatOpenAI usage continues to pick up credentials.
    # Also set API type/base/version so the underlying openai client will call Azure endpoints.
    os.environ["OPENAI_API_KEY"] = AZURE_OPENAI_API_KEY
    os.environ["OPENAI_API_TYPE"] = "azure"
    # OPENAI_API_BASE is used by some SDK versions — set it to the provided endpoint
    # Ensure the endpoint contains the scheme, e.g. https://your-resource.openai.azure.com
    os.environ["OPENAI_API_BASE"] = AZURE_OPENAI_ENDPOINT
    if AZURE_OPENAI_API_VERSION:
        os.environ["OPENAI_API_VERSION"] = AZURE_OPENAI_API_VERSION
    # Also set AZURE_* vars which some LangChain variants look for
    os.environ["AZURE_OPENAI_API_KEY"] = AZURE_OPENAI_API_KEY
    os.environ["AZURE_OPENAI_ENDPOINT"] = AZURE_OPENAI_ENDPOINT
    if AZURE_OPENAI_DEPLOYMENT:
        os.environ["AZURE_OPENAI_DEPLOYMENT"] = AZURE_OPENAI_DEPLOYMENT

    st.info("Azure OpenAI environment variables loaded. Using Azure OpenAI for LLM calls. ☁️")

# Initialize LLM and agent (matching base)
# llm = ChatOpenAI(model="gpt-4o-mini", temperature=0)
llm = AzureChatOpenAI(
    azure_deployment=os.environ.get("AZURE_OPENAI_DEPLOYMENT"),   # required: your deployment name
